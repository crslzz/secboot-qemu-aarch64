...
usage() {
    ...
    echo "  --nssdb: Path to NSS database (default is $NSSDB)"
...
import_cmd=(pk12util -i "$CERTS" -d "$NSSDB")
if [[ ! -z "$NICKNAME" ]]; then
...
for file in "${FILES[@]}"; do
    ...
    pesign -v --force -n "$NSSDB" -c "$NICKNAME" -s -i $file -o $file.signed
    mv $file.signed $file
done
